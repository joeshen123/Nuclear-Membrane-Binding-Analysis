...
class cell_tracking:
    def __init__(self, segmented_image_seq, intensity_image_stack, lmnb1_image_stack, smooth_sigma, x_vol, y_vol, z_vol):
        ...
        self.lmnb1_image_stack_raw = lmnb1_image_stack
        self.lmnb1_image_stack = np.zeros(lmnb1_image_stack.shape)
    ...
    def intensity_stack_smooth(self):
        ...
        for t in pb:
            ...
            lmnb1_img = self.lmnb1_image_stack_raw[t].copy()
            
            self.intensity_image_stack[t] = image_smoothing_gaussian_3d(img, sigma=self.smooth_sigma)

            struct_img = intensity_normalization(lmnb1_img,[0.5,8])
            self.lmnb1_image_stack[t] = image_smoothing_gaussian_3d(struct_img, sigma=self.smooth_sigma)

            # Use rolling_ball to remove background
            background = rolling_ball(self.lmnb1_image_stack[t], kernel=ellipsoid_kernel((20, 1, 1),0.2))
            self.lmnb1_image_stack[t] = self.lmnb1_image_stack[t] - background
    ...
    def create_table_regions(self):
        ...
        for n in pb:
            ...
            for region in measure.regionprops(labelled_slice):
                ...
                lmnb1_single = self.lmnb1_image_stack[n]
                lmnb1_image = lmnb1_single[mid_z]

                intensity_median = np.median(intensity_image[segmented_image_shell == True])

                intensity_background = np.median(lmnb1_image[mid_nucleus_image == True])
